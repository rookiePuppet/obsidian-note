...
对于图形程序来说，对一个物体应用多个变换是常见的，例如我们想先应用一个缩放矩阵，再应用一个旋转矩阵，这可以写成：

$$
v'=(RS)v
$$

这也就意味着，我们可以利用一个单独的相同规模的矩阵来表示对一个向量应用多种变换效果的矩阵序列。
...
于是，如果我们想绕任意轴a旋转，可以用$w=a$构建一个正交基，将其旋转到标准正交基$xyz$，绕z轴旋转，然后再从标准正交基旋转回$uvw$基。用矩阵形式表示为：
