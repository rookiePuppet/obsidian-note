...
斜平行视图只需要独立指定图像平面法线$w$和观察方向$d$即可，用$d$替代$-w$。
...
仅当$\beta>0,\ \gamma>0,\ 且\beta+\gamma<1$时，交点位于三角形内部。如果无解，要么是三角形退化（无效的三角形，例如三点共线、存在大于180°的角），要么是射线平行于三角形所在平面。
...
然而许多现实的表面会显示出光泽度，产生高光或镜面反射，并随着视点的改变而移动。由于Lambert着色并不产生任何高光，造成了不光滑的外表，所以一些着色模型会在Lambert着色（**漫反射部分**）的基础上添加**镜面反射部分**。

一个非常简单且广泛使用的镜面模型最早由Phong提出，后被Blinn改进成如今最普遍应用的形式。其思想是使在$v$和$l$关于表面法线对称时产生最亮的反射，即镜面反射，而当两个向量逐渐偏离这种镜面对称时，反射减弱。
...
\begin{align}
&for\ each\ pixel\ do\\
&\quad compute\ viewing\ ray\\
&\quad if\ ray\ hits\ an\ object\ with\ t\in [0,\infty)\ then\\
&\quad\quad Compute\ n\\
&\quad\quad Evaluate\ shading\ model\ and\ set\ pixel\ to\ that\ color\\
&\quad else\\
&\quad\quad set\ pixel\ color\ to\ background\ color
\end{align}
$$

在实际实现时，表面求交程序需要返回命中物体的引用，或者至少其表面法线和着色相关的材质属性，这通常是通过传递带有这些信息的记录或结构来完成的。

### 光线追踪程序的面向对象设计

光线追踪器的关键类层级就是组成模型的几何体，它们应该作为某个几何体类的子类并提供一个**hit**函数，父类通常叫做**surface**。重点是，任何可以被射线“击中”的东西都应该作为这个类层级的一部分。甚至是表面的集合也应该作为surface的子类，这包括一些高效的数据结构，比如包围盒体积层次结构。
...
这里的$(t_0,t_1)$是返回的碰撞点在射线上的区间，$rec$是通过引用传递的记录，包含了当碰撞发生时相交点对应的$t$和其他数据。$box$类型是一个3D“包围盒”，即两个点，定义了一个包围表面的轴对称盒子。例如对于圆形来说，该方法的实现为：
...
material是另一个有用的类，用于抽象材质行为。链接物体和材质的一种简单方法是，在surface类中添加一个材质的指针。纹理的处理是一个大问题，它应该作为材质的一部分还是存在于材质的外部呢？

## 阴影

光照来自于方向$l$，想象我们在表面的一个点$p$上，然后沿着方向$l$看，如果能看到某个物体，说明我们在阴影中。

![[Pasted image 20250616142657.png]]

决定某点是否在阴影内的射线叫做**阴影射线**。

为得到着色算法，我们添加一条if语句来检查点是否在阴影内。在一个简单直接的实现中，阴影射线会检测$t\in [0,\infty)$，但由于数值的不精确，会导致和表面相交。避免此问题的常见思路是将$t$设置为$t\in[\epsilon，\infty)$，$\epsilon$是一个正的很小的常数。

![[Pasted image 20250616143855.png]]

在Phong照明中实现阴影射线的代码如下：

![[Pasted image 20250616144001.png]]

## 完美的镜面反射

当观察者朝一面完美的镜子看去时，从方向$e$看到的画面和向方向$r$看到画面相同，向量$r$是通过Phong光照反射方程的一个变体得到的。

$$
r=d-2(d\cdot n)n
$$

![[Pasted image 20250616145508.png]]

在现实世界中，光在表面反射时会损失一定的能量，不同的颜色会有不等量的损耗。例如黄金反射黄光的效率高于蓝光，因此会造成反射物体的色偏。这可以通过在光线颜色计算中添加一个递归调用来实现：

$$
color\ c=c+k_mraycolor(p+sr,\epsilon,\infty)
$$

递归调用的问题就是它会永不终止，例如在房间中的一条光线会不停反弹。这可以通过添加一个最大递归深度来修复，另外仅当$k_m$不为零时才生成反射光线会更高效。

## 历史说明

光线追踪在计算机图形学的早期就被发明出来了，但是直到计算机算力充足后才被大量使用。

光线追踪的渐进时间复杂度低于基础的物体顺序渲染，虽然它被认为是一种传统的离线方法，但是实时光线追踪实现越来越普遍。
