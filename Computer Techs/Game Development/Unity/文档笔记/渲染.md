...
在URP渲染器资产中，可以设置Intermediate Texture为Auto，这样Unity只会在必要时使用中间纹理进行渲染，以减少使用的GPU带宽。

另外还可以：

- 减少贴花（Decal Renderer Feature）的使用，因为贴花需要额外的渲染通道来渲染，这样可以同时降低CPU和GPU处理时间。
- 剔除包含无用特性的Shader变体。
...
- 若使用Vulkan，Metal 或 DX12，启用Native RenderPass，这样URP会自动降低从内存中拷贝渲染纹理的频率。
- 若使用Forward或Forward+路径，在主机和PC平台上设置Depth Priming Mode为Auto或Forced，这样URP会使用深度纹理来避免对丢弃的片元执行像素着色器，但是在移动平台上如果不使用就要将其禁用。
...
要在渲染通道中声明并使用资源，必须调用渲染图系统API来获取资源的句柄，不能自己直接分配和释放资源。

渲染图系统仅在第一个渲染通道写入资源之前，为资源分配内存，在最后一个渲染通道读取完之后将资源释放。

可以通过两种方式创建资源：

1. 在渲染通道中创建资源（内部资源），其生命周期由渲染图系统管理，无法从渲染图实例外部访问它们。
2. 从渲染通道外部导入现有资源（外部资源），例如摄像机的后置缓冲区或者是希望被渲染图系统在多帧之间使用的纹理。
