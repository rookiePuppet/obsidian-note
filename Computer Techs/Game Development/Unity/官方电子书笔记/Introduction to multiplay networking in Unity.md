...
| 任务/系统 | RPC                          | NetworkVariable                      |
| ----- | ---------------------------- | ------------------------------------ |
| 库存管理  | 当物品被捡起或使用，被添加到背包或移除时，通知客户端   | 维护每一个玩家的背包，背包可以是一个同步物品列表的NetworkList |
| 战斗系统  | 执行战斗动作，如攻击或特殊移动，RPC可以实施伤害和效果 | 同步每一个玩家的生命值和状态                       |
| 环境交互  | 触发特殊动作如开门，或激活一个机制            | 维护可交互对象的状态，例如一扇门是打开的还是关闭的            |
| 任务目标  | 通知任务目标完成                     | 追踪任务目标的进度，存储已收集的物品或已完成的任务数量          |

## 面向多人游戏设计

在制作多人游戏时，适应“多人联网”的思维方式是很重要的。在单人游戏中很简单的机制，当加入NetworkVariable和RPC在多个设备上制造出相同行为时，常常变得更加复杂。

计划如何在客户端和服务器之间同步状态，使用NetworkVariable持续同步数据，使用RPC触发离散事件，然后只对必要的数据进行同步以最小化网络流量。

将单人游戏转换为多人游戏是可行的，但从一开始就以多人游戏进行设计一般会更加高效。尽早确定哪一些对象和动作是服务器持有的，哪一些可以被客户端管理。虽然服务器权力具有最高的安全性和一致性，但是还要采用客户端权力对特定操作降低时延。

# 网络时延和性能

质量低下的带宽和不稳定的网络连接会导致玩家移动卡顿，帧率不稳定，以及可观的输入延迟。

即使在UnityTransport提供的额外可靠性下，UDP数据仍然可能丢失、失序或损坏，这些都是时延的潜在来源。

## 模拟时延

为了在开发过程中理解和减轻时延的影响，可以使用UnityTransport的Debug Simulator或Network Simulator来模拟各种网络状况。

### Unity Transport Debug Simulator

可以调整UnityTransport组件的Debug Simulator来引入人工时延、抖动和丢包，当选中场景中的NetworkTransport对象时，可以在Inspecotor中找到这些设置。

- Latency（时延）：添加延迟来模拟缓慢的网络连接，例如设置为100ms来模拟中度网络延迟
- Jitter（抖动）：引入时延变化来模拟不稳定的网络环境，例如设置为50ms来检验不稳定的连接如何影响游戏
- Packet Loss（丢包）：指定丢包概率，模拟低质量的网络连接

### Network Simulator

来自Multiplayer Tools包中的Network Simulator也可以测试不理想的网络条件，它可以让模拟网络事件更加便捷，如网络断连、延迟尖峰和丢包。

### 其他网络调节器

Debug Simulator和Network Simulator只能在编辑器中使用，对于已构建的运行时，也有一些网络调节器可以用来模拟时延，例如Windows的clumsy和macOS/iOS的Network Link Conditioner。

处理时延对程序性能的影响是多人游戏开发中的最大挑战之一，幸运的是有一些策略可以帮助缓解时延的影响。

## 客户端侧插值

缓解时延影响的其中一种方式就是客户端侧插值，在该方法中，客户端会有意地延迟一个插值周期再渲染，而非立即渲染。

在客户端-服务器拓扑模式下，客户端一般在服务器之后半个往返时间后渲染一次状态，客户端侧插值会在此基础上额外增加一个延迟。

通过稍微延后运行，客户端就可以缓冲来自服务器的状态更新。直到渲染下一次更新时，客户端根据最近两次服务器tick计算出一个插值状态。

缓冲区可以让客户端渲染稳定的客户端更新，即使来自服务器的tick是在抖动的不稳定速率下到达的，插值状态能够掩盖细微的时延和抖动。

在Netcode for GameObjects中，客户端侧插值作为NetworkTransform的一个选项，开启Interpolation可以对相关GameObject启用位置、旋转和缩放插值。

## 客户端侧预测和提前反应

在之前的例子中，ClientNetworkTransform给予了客户端对玩家移动的直接控制权，使游戏响应感觉更迅速。但是在许多游戏中，因为游戏设计、公平性和安全性等因素，不能给予客户端权力。

### 为什么需要服务器权力

在竞技类游戏中，公平性是至关重要的，客户端权力会给予玩家操控客户端侧代码和数据的作弊能力。具有复杂玩家交互和共享世界的游戏，一般需要服务器权力来确保数据完整，防止篡改，维持所有玩家一致的体验。

服务器权力可能会加重时延，因为客户端必须发送其输入给服务器，服务器处理输入来模拟游戏并计算游戏状态，仅当客户端接收到模拟的结果后才能显示下一帧。

### 客户端侧预测原理

客户端侧预测是一种解决由服务器权力引入的延迟的方案，客户端会在一瞬间预测未来的游戏状态并可见地更新游戏，因为客户端并不知道来自服务器的真实状态，所以叫“预测”。

这样，客户端就能够对玩家操作进行即时的视觉反馈，让游戏反应感觉更迅速。

同时，客户端将玩家的操作通过数据包发送给服务器，服务器处理输入，模拟游戏状态，再将权威性的状态传回客户端。

## 修正与回滚

客户端会将服务器权威状态和预测状态进行对比，找出不同之处。如果两者足够接近，则无事发生；如果两者差异明显（desync），客户端必须决定如何修正以匹配服务器权威状态。这个过程叫做修正（Reconciliation）。

为了处理时延补偿，客户端会保存历史输入记录，和一定帧数量的预测状态。当不同步时，客户端可以回滚到从服务器已知最近的正确状态，然后从这个点开始重新模拟游戏，让客户端回到和服务器同步的状态。

### Netcode for GameObjects中的客户端侧预测

Necode for GameObjects使用的是简化的客户端侧预测和修正模型，使用这些组件：

- `AnticipatedNetworkVariable<T>`：泛型组件，可用于标量或简单数据类型。适合非变换属性，例如生命、比分或物品状态。
- `AnticipatedNetworkTransform`：专用于变换数据，包括位置、旋转和缩放

客户端预测还需要考虑“过时数据”，即在客户端上一次请求之前发生的操作反映的服务器更新，Netcode for GameObjects通过`StaleDataHandling`属性提供两种方式来处理：

- `StaleDataHandling.Ignore`：忽略过时数据，保持预测值。当状态迅速改变，造成视觉闪烁时有用
- `StaleDataHandling.Reanticipate`：像其他服务器更新一样对待过时数据，触发回滚和重新预测，通过重放玩家输入来维持一致性

为了防止预测值与服务器值不同时的不连贯视觉更新，可使用`Smooth`方法。

真正的回滚和修正需要一个确定性物理系统，确保相同的输入总是产生相同的结果，这对准确回滚和重新模拟游戏状态是关键的。

> [!NOTE] 确定性物理
> 确定性物理系统能保证给予它相同的初始条件和输入，总是会产生相同的模拟结果。
> Netcode for GameObjects使用Unity内置的物理引擎，是非确定性的。
> Netcode for Entities支持Unity Physics和Havok Physics，都提供确定性模拟能力。

### Netcode for Entities中的客户端侧预测

Netcode for Entities提供处理客户端侧预测和延迟补偿的更高级工具，每一个实体在客户端和服务器上运行相同的模拟代码，使客户端基于玩家输入预测游戏状态，带来即时的反馈。

当客户端接收到服务器最新的快照时，它会更新所有被预测的实体。在应用快照之后，客户端会运行一个叫做`PredictedSimulationSystemGroup`的模拟，从最早的tick开始逐一处理，回滚并重新模拟游戏状态。这个过程能帮助客户端修正差异，维持与服务器权威状态的同步。

Netcode for Entities也支持以下高级物理特性：

- 多物理世界（Multiple physics worlds）：允许仅限本地的物理模拟，无需在网络上复制的
- 自定义物理代理（Custom physics proxies）：开启对只出现在客户端上的物理对象的虚拟交互
- 确定性物理模拟（Deterministic physics simulation）：确保客户端和服务器在输入相同的情况下，产生相同的模拟结果

`GhostPredictionSmoothingSystem`可以通过在预测状态和权威状态之间过渡，帮助平滑预测错误。

| 功能   | Netcode for GameObjects    | Netcode for Entities                               |
| ---- | -------------------------- | -------------------------------------------------- |
| 预测方法 | 客户端预测：客户端预测服务器响应           | 完整客户端预测：客户端运行和服务器相同的模拟代码                           |
| 时延缓解 | 预测服务器结果，平滑过渡               | 回滚和重新模拟，修正不同步                                      |
| 快照   | 未明确使用                      | 使用快照表示特定时刻的游戏状态                                    |
| 延迟补偿 | 提供StaleDataHandling选项的简化模型 | 参考碰撞世界的综合延迟补偿                                      |
| 物理交互 | 有限的客户端侧预测，使用预测的变换          | 支持预测的和仅客户端的物理世界之间进行交互                              |
| 平滑   | 对预测值使用Smooth方法             | GhostPredictionsmoothingSystem用于平滑预测状态和权威状态之间的平滑过渡 |
| 用例   | 适用于需要即时视觉反馈的简单游戏           | 适用于需要准确状态同步的复杂游戏                                   |
> [!NOTE] Netcode for Entities术语
> Netcode for Entities使用ECS和DOTS，区别于MonoBehaviour工作流。
> - Entity：实体是表示单个GameObject或组件的基本数据单元，只包含数据，是轻量化的
> - Game world：游戏世界指的是游戏发生的整个环境，包括所有的实体、它们的状态和规定它们交互的规则
> - Collision world：碰撞世界是游戏世界中所有物理对象的状态，包括它们的位置、速度和交互，用于碰撞检测和物理模拟
> - Snapshot：快照是表示某一特定时刻的游戏状态的数据集，客户端和服务器通过快照更新游戏世界，以周期性地保持同步
> - Ghost：幽灵是在客户端和服务器上复制的联网实体，服务器每帧都会发送所有幽灵当前状态的快照给客户端。幽灵用于在服务器和客户端之间同步实体的状态，确保所有玩家看到的游戏世界是一致的
> - Predicted ghost：预测幽灵是本地模拟的客户端侧实体，提供对玩家操作的即时视觉反馈，降低感知时延
> - Interpolated ghost：插值幽灵是客户端上的服务器侧实体表示，客户端根据从服务器接收到的快照显示其状态，混合它们以减少时延带来的抖动
