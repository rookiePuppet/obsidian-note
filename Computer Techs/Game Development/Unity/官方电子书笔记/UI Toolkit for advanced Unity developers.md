...
Unity6支持一种混合的数据绑定工作流，将UI Builder的可视化设置和脚本的灵活性结合起来。

和在UXML中硬编码数据源不同的是，你可以只指定数据源类型，将实际数据源留空。UI Builder会对这种未完成的绑定使用一个图标进行标记，意味着数据源的类型和路径都已设置，但数据源还未赋值。在运行时，只需要一行代码对数据源进行赋值即可。

```CS
myElement.dataSource = myNewDataSource;
```

这里将myNewDataSource赋值给myElement解决了UXML中定义的占位绑定，让UI自动更新，消除了重复的SetBinding调用并保持了UXML的灵活度。

注意：如果UXML文件明确设置了数据源，那么绑定会变成固定的，无法在运行时改变。

## 类型转换器

类型转换器可以将原始数据转换为更加用户友好的格式显示在UI上，它们作为数据源和UI之间的中介，将数据转换为更加直观的格式。例如，类型转换器可以将弧度转换为角度或将原始的生命值转换为颜色。

Unity支持以下两种类型的转换器：

- 全局转换器：应用于任何需要特定类型转换的绑定，例如，全局转换器可以将浮点的生命百分比转换为颜色值或将Color对象转换为StyleColor类型
- 预绑定转换器：应用于需要更精细控制的特定数据绑定

最佳实践：

- 最小化分配：保持转换委托轻量化，避免不必要的性能开销
- 保持简单：编写简单、专注的转换器，避免嵌入复杂或消耗大量资源的逻辑
- 将转换集成到数据源：在数据源自身中处理基本的转换，将数据转换器保留给特定于UI绑定的转换

## 优化数据绑定

重复和过度的数据绑定会带来不必要的更新，降低游戏性能，因此高效的数据绑定对于复杂的和资源密集型界面来说尤其重要。

默认情况下，运行时绑定系统逐帧更新UI元素，大量的绑定可能会成为项目的性能瓶颈。

1. 处理值类型：值类型的数据源需要注意装箱开销，因为dataSource属性是作为object来处理的，所以当使用值类型时要减少不必要的绑定或重复更新。
2. 降低开销：找出更新相同元素多次的绑定和极少改变的数据，将这些不必要的绑定进行合并或移除，尽可能使用简单的数据结构；考虑对耗时计算进行缓存或预计算；确保绑定用于需要频繁更新的元素，对于不需要持续同步的元素，直接赋值或只通过触发事件来更新。
3. 使用更新触发器（Update Trigger）：绑定刷新基于更新触发器，它决定UI和数据源同步的频率。针对每一个元素的需求，调整更新触发器，在响应速度和性能之间取得平衡。
4. 版本管理与变化追踪：为了减少不必要的更新，可以将版本管理和变化追踪集成进数据源中。使用`IDataSourceViewHashProvider`和`INotifyBindablePropertyChanged`接口让数据绑定更高效。

# 本地化

Unity6将`Localization`包直接集成到了UI Toolkit，简化了本地化过程。

Unity本地化的关键是`Locale`类，它表示一种特定的语言，处理区域特定的细节，比如货币和数字格式化。

以下是Localization包的一些关键特性：

- 字符串本地化：`LocalizedString`类管理在切换语言时自动更新的文本，使用`SmartStrings`可以添加占位符，处理复数等其他语言特定细节。
- 资产本地化：根据语言切换纹理或其他资产。
...
构建复杂的游戏UI通常意味着需要维护界面元素的一个巨大层级结构，即使是细微的低效，在大量元素的堆积下也可能造成运行时的卡顿，对玩家体验带来负面影响。

大多数的性能问题都可以通过一些优化手段得到解决，虽然Unity6在UI Toolkit上提供了更好的开箱即用的性能，但是一个真正高效的界面仍然需要开发者付出努力。

大部分的优化工作通常归结为消除不必要的开销和减少绘制调用。

## 更新机制

![[Pasted image 20260204094202.png]]

视觉树包含在运行时对样式、布局和内容进行响应的一些更新机制，任何一方面都会影响性能。

| 更新机制    | 描述                           | 发生时机                              | 性能影响                  | 优化思路                    |
| ------- | ---------------------------- | --------------------------------- | --------------------- | ----------------------- |
| 样式解决    | 通过应用USS选择器和**样式**，决定元素最终的外观  | 当类或样式被改变时触发，例如添加一个样式类或修改颜色        | 大型和深度嵌套的层级结构在此过程中开销巨大 | 最小化样式变化                 |
| 布局重计算   | 调整元素的**位置和尺寸**，使它们在UI层级中正确适配 | 当元素的大小、位置或对齐发生改变时触发，例如缩放一个面板或移动元素 | 频繁的布局更新会浪费性能          | 使用transform来做动画，或直接更改位置 |
| 顶点缓冲区更新 | 更新渲染UI元素的**几何**形状，比如矩形或圆角    | 当一个元素的几何发生改变时触发，例如添加圆角或修改边界       | 更新顶点缓冲区是大量消耗资源的       | 避免频繁的几何更新               |
| 渲染状态变化  | 改变**渲染状态**，如用于绘制元素的纹理和混合模式   | 通过像遮罩这种特性，或扰乱批处理的特殊纹理触发           | 过的状态改变会增加CPU负担。       | 批处理，限制特殊纹理或遮罩的使用        |
## 元素批处理

在渲染用户界面时，每一个视觉元素都需要发送指令给GPU。UI Toolkit通过批处理优化这些绘制调用，它会将对GPU需求相同的元素组合在一起处理，批处理能够显著减少CPU和GPU通信时的性能损耗。

要将元素有效地合批，元素们必须共享相同的GPU状态，即相同的着色器、纹理、网格数据和其他GPU特定的参数，例如使用相同字体和样式的文本元素序列可以合并在一起，但是在它们之间插入一张使用不同GPU设置的图像，就会导致一个新的批次。

## 顶点缓冲

在UI Toolkit中，顶点缓冲区存储用于渲染UI的几何（顶点）。当一个UIDocument在运行时创建一个面板时，它会预分配一个单独的顶点缓冲区来处理视觉元素。可以把这个缓冲区视为元素的“堆内存分配器”，当元素被添加到UI上时动态地分配内存。

如果UI超出了顶点缓冲区的容量，就会创建额外的缓冲区，这会使批处理碎片化，增加绘制调用数，极大降低性能。

可以在面板设置中调整`Vertex Budget`，配置顶点缓冲区的初始大小来解决该问题。其默认值为0，Unity会自行决定缓冲区大小，但是对于复杂UI，手动提升该值可以提升性能。

## 超级着色器和八纹理限制

UI Toolkit将所有UI渲染功能整合进了一个多功能的“超级着色器”，使用动态分支在运行时选择恰当的渲染路径，而不是依赖多个着色器变体。这可以减少着色器切换，从而降低CPU负担，但分支逻辑会增加一些GPU开销。

超级着色器的一大特性是在同一个合批中支持最多八张纹理，让使用不同纹理的元素也能在同一个绘制调用下渲染。当UI中使用的纹理数小于等于8时，Unity将它们合并为一个绘制调用，然而一旦超过限制，每一张纹理都将变成一个单独的绘制调用。

## 动态纹理图集

在多张纹理之间切换会导致UI Toolkit打断合批，增加绘制调用。常用的方案是纹理图集，将小纹理组合为一张大纹理，Unity会将一个图集视为单张纹理，从而减少绘制调用。

UI Toolkit无缝集成了2D Sprite Atlas，这是静态或预定义内容的一个好选择，它的局限性在于无法处理运行时生成的纹理，而且需要一些提前的设置和布局，消耗时间。

动态纹理图集能高效地将多张图像合并到一张纹理，减少状态改变。可以在面板设置中配置图集设定，在Dynamic Atlas Viewer中查看图集布局。

需要注意的是，如果UI经历了大规模变化（比如随时间增加或移除了很多纹理），图集可能变得碎片化。在这种情况下，使用`ResetDynamicAtlas` API可以将图集恢复到初始状态。

2D Sprite Atlas和Dynamic Texture Atlas可以在UI Toolkit中同时使用，将Sprite Atlas用于静态、预定义的内容，而动态图集用于运行时驱动的UI内容。

## 遮罩

UI Toolkit使用模板缓冲来实现遮罩，由于模板缓冲是GPU状态的一部分，改变遮罩设置会导致合批打断。

小心逐层叠加的遮罩会增加复杂性，因为每一层嵌套都需要模板缓冲来追踪额外状态，会增加GPU开销。

UI Toolkit支持两种类型的遮罩：

- 矩形基地遮罩（Rectangular-based masking）：矩形遮罩使用基于着色器的操作，不会引起GPU状态改变，可以保留合批一致性。该技术不使用模板缓冲，因此可以无视深度限制地嵌套矩形遮罩。
- 圆角和复杂遮罩（Rounded Corners and Complex Masks）：圆角和其他复杂形状的遮罩需要模板缓冲操作，每一层遮罩都有可能打断合批。该技术最多支持7层遮罩嵌套。

优化遮罩元素的性能：

- 尽可能使用矩形遮罩，避免模板操作
- 最小化遮罩嵌套深度，减少模板重计算
- 尽可能对一个父元素使用单个遮罩，而不是对子元素使用多个遮罩
- 应用`Mask Container `usage hint优化模板状态设置

## 动画和过渡

虽然UI Toolkit的USS过渡提供简单的属性动画，但是改变布局属性会触发耗时的布局重计算，对此可以尝试采取一些策略来优化。

首先，在布局属性变化上优先使用基于transform的动画，即使用translate、scale或rotate，而不是width、height、top或left。transform操作是直接在GPU上处理，无需重新计算布局，带来更流畅的动画。

其次，可以对需要动画的视觉元素启用**usage hint**。**DynamicTransform**会让UI Toolkit在GPU上处理位置和变换更新，跳过耗时的顶点数据重计算。对于包含多个具有动画效果的子元素的父容器，**GroupTransform**会对父元素应用单个变换，GPU会将变换高效地传递到所有子元素，从而优化大型组合的动画。

另外，一般应避免在大型层级结构的UI动画期间切换类改变样式。类改变会触发耗时的样式重计算，尤其在复杂的UI结构中，通过修改内联属性直接更新样式可以最小化计算开销。

最后，使用Frame Debugger监视动画性能，验证这些优化是否达到预期。
...
隐藏元素时，只改变不透明度或移到屏幕外的性能不总是最好的，元素即使在隐藏时，也会参与布局计算、样式更新和数据绑定，仍然可能影响性能。

以下是隐藏元素的不同方式，每种都有其权衡。

![[Pasted image 20260204155356.png]]

将元素设置不透明度为0或移出屏幕，仍然对GPU和布局系统可见，渲染开销中到高，这两种方法对过渡有帮助，但不会降低内存和布局开销。

将元素visible属性设置为false可阻止渲染，但其仍保持为布局的一部分，这是一种在使用模板内存时临时隐藏元素的折中办法。

将style.display属性设置为DisplayStyle.None会完全停止渲染和布局更新，有更高效的性能，但是将元素显示回来时也会带来重新计算布局的开销。

对于不频繁显示的元素，例如对话框或设置面板，可使用`RemoveFromHierarchy`将它们移除。只是需要注意当元素重新放回时，由于布局必须完全重建，会引发更剧烈的性能骤降。

## 重复绘制（Overdraw）

当元素重叠时，会引起大量的重复绘制，即每个像素会被多次处理的情况。这对于使用超级着色器的UI Toolkit来说开销巨大，每一次重叠元素都增加了复杂度。堆叠多层透明或半透明元素会进一步影响性能。

一些能减缓过度绘制性能影响的策略：

- 使用`style.display=DisplayStyle.None`来隐藏元素，而非`style.opacity=0`
- 移除或隐藏所有被完全覆盖的元素，而不是相互之间堆叠多个元素
- 应对可滚动内容时，通过ListView实现虚拟化。ListView只会对屏幕上可见的元素进行渲染，更加高效
- 可以设置`style.overflow=Overflow.Hidden`在特定区域裁剪内容，减少可视范围外不必要的渲染
