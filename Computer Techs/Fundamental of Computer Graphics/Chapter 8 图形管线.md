...
和光线追踪一样，如果对每个像素是否在图元内部做全有或全无的判断，光栅化也会产生锯齿状线条和三角形边缘。而解决方案就是允许像素部分被图元覆盖，这样的模糊有助于视觉质量。

![[Pasted image 20250701142952.png]]

可以通过将每个像素值设置为该像素所在方形区域内的像素平均颜色来生成抗锯齿图像，这种方法称为**盒式滤波**。

实现盒式滤波最简单的方法是**超采样**，先在高分辨率下创建图像，再进行降采样。

超采样的代价非常大，由于导致混叠的锐利边缘通常是由图元的边缘引起的，而非图元内部着色的突变，因此一种广泛使用的优化方法是在比着色更高的频率下采样可见性。在具有片元着色的系统中，通过为每个片元额外存储覆盖掩码和一组深度值来实现**多重采样抗锯齿（MSAA）**。

> [!NOTE] MSAA基本流程
> 1. 为每个像素定义多个采样点。如4xMSAA有4个采样点，均分布在单个像素内部。
> 2. 高频率可见性测试。对每个采样点分别进行深度测试和覆盖测试。
> 3. 低频率着色计算。对每个像素只计算一次着色，多个采样点共用该结果。
> 4. 解析。输出颜色=被覆盖采样点的颜色加权平均。

## 剔除图元以提高效率

对象顺序渲染的优势是只需要对场景中的所有对象遍历一次，但是对于复杂场景来说，在某一时间内可能有大量不可见的物体，没必要浪费性能在这些对象上面。

识别并丢弃这些不可见物体的过程就叫做**剔除**，有三种常见的剔除策略（通常一起使用）：

- 视体剔除：移除位于视体之外的几何体
- 遮挡剔除：移除位于视体中但是被其他物体遮挡的几何体
- 背面剔除：移除朝向摄像机相反方向的图元

### 视体剔除

可以把多个三角形用一个包围体包裹起来，然后检查该包围体是否位于视体的外部，如果完全位于外部，那么说明被包裹的所有三角形都位于视体外部。

例如用一个球体包围1000个三角形，球体中心为$c$，半径为$r$，检查球体是否位于裁剪平面$(p-a)\cdot n=0$的外部，这相当于检查球心到平面的有向距离是否大于$+r$，即$\frac{(c-a)\cdot n}{||n||}>r$。

注意，即使所有三角形都在平面外部，球体也可能与平面重叠。

### 背面剔除

当多边形模型闭合时，这意味着它们自身是一个没有缝隙的封闭体积，一般会假设它们具有朝向外的法向量。对于这类模型，朝向视线方向的多边形肯定会被与面对视线的多边形所覆盖，因此这些多边形可以在管线开始之前被剔除。
