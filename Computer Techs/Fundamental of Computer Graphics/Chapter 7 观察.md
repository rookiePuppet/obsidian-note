...
其中x和y都被z除了，并且由于n和z都是负的，x和y并没有产生反转。

该变换还保留了z值在n~f之间的相对顺序，以便于之后做深度排序，这对于隐藏表面消除尤为重要。

![[Pasted image 20250623174035.png]]

有时我们想得到$P$的逆，例如当我们希望对屏幕上的物体做选取操作时，需要将屏幕坐标乘以z来变换回原空间。
...
由于齐次向量乘以一个标量不会改变其意义，我们还可以写成更简洁的形式：
...
在考虑正交投影矩阵$M_{orth}$的情况下，透视矩阵仅仅是将锥台形状的透视视体映射到轴对齐盒子的正交视体。

透视矩阵的美在于，一旦我们将其应用，然后再使用一个正交变换就可以得到标准视体。于是，所有的正交机制都能奏效，而我们只需要额外添加一个矩阵和一个除以$w$的操作。

那么在透视投影中，$l,r,b,t$应指定为什么呢？因为透视矩阵不改变$(z=n)$平面上的$x,y$值，所以我们可以在$(z=n)$平面指定$l,r,b,t$。


$$
M_{per}=M_{orth}P=\left[\begin{matrix}
...
为了将透视矩阵集成进正交投影结构中，我们只需替换$M_{orth}$为$M_{per}$，因此透视投影的完整矩阵为：
...
透视变换的一个重要性质就是它将直线变换为直线，将平面变换为平面，此外它将视体中的线段变换为标准视体中的线段。

假设线段$q+t(Q-q)$被一个4x4矩阵$M$变换，线段上的点可能具有变化的齐次坐标：

$$
Mq+t(MQ-Mq)\equiv r+t(R-r)
$$

齐次化的3D线段为$\frac{r+t(R-r)}{w_r+t(w_R-w_r)}$，如果能写成$\frac{r}{w_r}+f(t)(\frac{R}{w_R}-\frac{r}{w_r})$的形式，那么所有的齐次化点都位于一条3D直线上。

透视变换将线段变换为线段的附带效果就是，它将一个三角形的边和顶点变换到另一个三角形的边和顶点，也就是说它将三角形变换为三角形。

## 视野

虽然我们可以使用$(l,r,b,t)$和$n$值指定任意窗口，但有时如果视角穿过窗口中心，则可以用一种更简单的方式，即假定$l=-r,\ b=-t$。

如果继续约束像素为方形，那么$r$和$t$的比例一定和横纵像素数的比例相同：

$$
\frac{n_x}{n_y}=\frac{r}{t}
$$

![[Pasted image 20250625181236.png]]

一般用**视野**来表示图中的$\theta$，有时也叫做垂直视野。
