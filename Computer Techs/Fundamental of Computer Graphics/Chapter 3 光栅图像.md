...
![[Pasted image 20250612093440.png]]

图像中所有像素的$\alpha$值可能存储在一张单独的灰度图中，也叫做**透明度遮罩**（alpha mask 或 transparency mask）。也有可能将该信息作为一张RGB图像的第四个通道进行存储，该通道称为**透明度通道**，这样的图像称为RGBA图像。

### 图像存储

为了降低存储需求，大多数图像格式都允许某种程度的压缩，这种压缩可能是无损或有损的。流行的图像存储格式包括：

- jpeg：有损格式，基于人类视觉系统阈值压缩图像块。
- tiff：常用于存储二值图像或无损压缩8-bit或16-bit RGB。
- ppm：非常简单的无损、无压缩格式，常用于8-bit RGB图像。
- png：无损格式，拥有一套很好的开源管理工具。

由于压缩和格式变体的存在，为图像编写输入/输出程序可能相当费事。幸运的是，开发者通常可以依赖库函数来读写标准文件格式。在追求简洁性高于效率的快速开发场景中，原始ppm格式文件是简单高效的选择——只需将内存中存储图像的数组转储至文件，并在文件开头添加对应的文件头即可。